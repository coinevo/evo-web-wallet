    ...
    <v-card-text>
      ...
      <div v-for="(tx, id) in txList" :key="id">
        <v-layout style="border-bottom: 1px dashed">
          <v-flex xs10>
            {{ $t('view_tx.tx') }}
            <v-btn small light :href="node.getTxExplorerUrl(tx.id)" target="_blank">
              {{tx.id}}
            </v-btn>
          </v-flex>
          <v-flex xs2>
        ...
        <v-layout style="border-bottom: 1px solid">
          <v-flex xs4>
            <v-chip label outline class="white--text">
              {{ $t('view_tx.total_in') }}{{Wallet.changeUnitFromSatTo1(tx.inputValue)}}
            </v-chip>
          </v-flex>
          <v-flex xs4>
            <v-chip label outline class="white--text">
          ...
          <v-flex xs4>
            <v-chip label outline class="white--text">
              {{ $t('view_tx.total_out') }}{{Wallet.changeUnitFromSatTo1(tx.outputValue)}}
            </v-chip>
          </v-flex>
        </v-layout>
      </div>
